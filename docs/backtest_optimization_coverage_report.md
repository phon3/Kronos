...
The best targeted multi-target result used schedule 3:

- Position size: 50%
- Stop loss: 8%
- Loss multiplier: 1.5x
- Daily loss limit: 3%
- OOS return: 11.98%
- OOS buy-and-hold: 11.75%
- Excess return: 0.23 percentage points
- Reported exits: 30

The best no-fixed-TP variant returned 12.07%, or 0.32 percentage points over buy-and-hold, with 29 reported exits.
...
- Sample count 1 produced one qualified behavior.
- Sample count 3 produced no benchmark-beating candidate.
- Best qualified excess return was 0.32 percentage points.
...
- Two behaviorally distinct candidates qualified.
- Both required 50% sizing and 1.5x loss escalation.
- Excess return remained only 0.23–0.32 percentage points.
...
2. The trend detector to agree with the predicted direction.

This confirmation removes most Kronos signals. The final qualified Combined behavior reported 29 exits, while Kronos-only configurations ranged from 12 to 398 exits depending on threshold.
...
- Combined variants returning 14–20% with fewer than ten exits;
...
|---|---|---|---:|---|---|
| Combined | BTC/USD 1h | `btc_usd_1h_prod` | 12.07% vs 11.75% hold | 29 reported exits, 3/3 profitable folds | Yes, marginal |
...
Within the tested 1h model/data set, Combined is the only mode with a qualified behavior, but its OOS advantage is too small and too dependent on aggressive sizing to establish reliable live alpha. The next evidence-producing work is matched 15m and 1d model/data evaluation plus longer regime coverage, not relaxing ranking gates or selecting low-trade high-return configurations.
